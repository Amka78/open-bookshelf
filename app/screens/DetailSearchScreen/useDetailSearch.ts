...
export function useDetailSearch(initialQuery: string) {
  ...
  const buildNextQuery = useCallback(
    (parts: string[], operators: Record<string, QueryOperator>): string =>
  ...
  const onNodePress = useCallback(
    ...
    [buildNextQuery, itemOperators, pendingQuery],
  )

  const onItemOperatorChange = useCallback(
    (itemKey: string, op: QueryOperator) => {
  ...
  const onItemCalibreOperatorChange = useCallback(
    (categoryKey: string, value: string, newOp: CalibreFieldOperator) => {
    ...
    [buildNextQuery, itemCalibreOperators, itemOperators, pendingQuery],
  )
